...
const argUtils = {
     ...
     * 
     * @param {Object|String} schema The schema to use when performing checks.
     *        This argument can be specified as a string, which would be the
     *        name of the schema file in the schemas directory, or as the 
    ...
    buildSchemaChecker: function(schema, message) {
        if (typeof schema === 'string') {
            schema = require(_path.resolve(__dirname, '../schema', `${schema}.json`));
        } else if (!schema || (schema instanceof Array) || typeof schema !== 'object') {
        ...
        return (target) => {
            if (!schemaValidator(target)) {
                ...
                const error = new Error(`${message}. Details: [${schemaErr.dataPath||'<root>'}: ${schemaErr.message}]`);
                return error;
            }
            return false;
        };
